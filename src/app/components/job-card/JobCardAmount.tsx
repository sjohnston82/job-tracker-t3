...
const JobCardAmount = () => {
  ...
  const { setTotalJobs, activeJobs, archivedJobs } = useJobAppStore();
 

const { data, isLoading, error } =
  api.jobApplicationsRouter.getAllJobs.useQuery(
    { userId: user?.id ?? "" }, // Ensure a valid user ID is passed
    {
...
if (error) return <p>Error: {error.message}</p>;
  ...
  return (
    <div className="mx-auto text-center">
      <p className="text-lg font-bold">Showing {showingActive ? activeJobs?.length : archivedJobs?.length} {showingActive ? "Active" : "Archived"} Job Applications</p>
    </div>
  );
};
